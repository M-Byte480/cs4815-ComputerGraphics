...
void barChart (void)
{
	...
	for (k = 0; k < 12; k++){
		width = k * 50 * winWidth * 0.0015;
 		glRecti (offsetX+(20 + width), offsetY + 75, offsetX+(width + 40), offsetY + (dataValue [k] * height));
	}
	...
	for (month = 0; month < 12; month++) {
		glRasterPos2i (offsetX + xRaster, offsetY + yRaster);
...
void lineChart (void)
{
	...
	for (int k = 0; k < 12; k++)
	      glVertex2i(offsetX + ( x + k*50 * w),offsetY+ ( dataValue[k]*h));
	glEnd();
	...
	for (int k = 0; k < 12; k++)
	  {
		glRasterPos2i(offsetX + (25 + k*50 * w), offsetY + ((dataValue[k]-4) * h));
	...
	for (int m = 0; m < 12; m++)
	  {
		glRasterPos2i(offsetX+ (20 + m*50 * w), offsetY+ (yRaster * h));
...
void pieChart (void)
{
    ...
    circCtr.x = winWidth / 2 + offsetX;                  // Circle center position.
    circCtr.y = winHeight / 2 + offsetY;
...
void winReshapeFcn (int newWidth, int newHeight)
{
	...
	glViewport(viewX, viewY, winWidth, winHeight);
	glutPostRedisplay();
}
...
void processMenuEvents(int opt)
{
	switch(opt){
        ...
        // Zoom
		case ZOOM_OUT_ONE: scaleFactor -= 0.1; break;
		case ZOOM_IN_ONE: scaleFactor += 0.1;  break;
		case ZOOM_OUT_HALF: scaleFactor *= 0.5; break;
		case ZOOM_IN_DOUBLE: scaleFactor *= 2; break;

        // Move
        case MOVE_UP: offsetY += 3 ; break;
        case MOVE_DOWN: offsetY -= 3 ; break;
        case MOVE_LEFT: offsetX -= 3 ; break;
        case MOVE_RIGHT: offsetX += 3 ; break;
	}
...
// Special function for the 'special' keys such as arrows
void specialFcn(int k, int x, int y){
    if(k == GLUT_KEY_UP) offsetY += 3;
    if(k == GLUT_KEY_DOWN) offsetY -= 3;
    if(k == GLUT_KEY_LEFT) offsetX -= 3;
    if(k == GLUT_KEY_RIGHT) offsetX += 3;

    displayFcn();
}

void createGlMenus(){
	int menu, displays, zoom, move;
    ...
    move = glutCreateMenu(processMenuEvents);
